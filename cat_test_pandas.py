if __name__ == "__main__":
    ...
    df = pd.read_csv("perf2.txt",
                     delimiter="|", names=cols, dtype=all_but_dates, parse_dates=dates_only)
    print(df.dtypes.describe())
    print("type(df.dtypes[2]) = ", type(df.dtypes[2]))
    print("df.dtypes[2] = ", df.dtypes[2])
    print("type(df.dtypes[2].categories) = ", type(df.dtypes[2].categories))
    print("df.dtypes.categories = ", df.dtypes[2].categories)
    s = df["servicer"]
